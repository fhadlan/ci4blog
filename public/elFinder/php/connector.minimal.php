...
$user_trash_folder = $user_folder_path . "/.trash";
if (!is_dir($user_folder_path)) {
	mkdir($user_folder_path, 0777, true);
}
if (!is_dir($user_trash_folder)) {
	mkdir($user_trash_folder, 0777, true);
}
$opts = array(
	//'debug' => true,
	'roots' => array(
		...
		array(
			'id'            => '1',
			'driver'        => 'Trash',
			'path'          => $user_trash_folder . '/',
			'tmbURL'        => dirname($_SERVER['PHP_SELF']) . '/' . $user_trash_folder . '\/.tmb/',
			'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
